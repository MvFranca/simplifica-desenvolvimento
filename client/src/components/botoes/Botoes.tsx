...
const Botoes = ({ id, conteudo}: props) => {
  ...
  useEffect(()=> {
    ...
    if(modalTrofeu){
      ...
      axios.put(`http://localhost:8000/api/points/user/${idUser}/diamantes`, {pontos:pontos}).then((res) => {
        console.log(res)
      }).catch((err:Error) => {
        console.log(err)
      })

      axios.put(`http://localhost:8000/api/points/user/${idUser}/fogo`, {fogo: fogo}).then((res) => {
        console.log(res)
      }).catch((err:Error) => {
        console.log(err)
      })
